...
export default function HomePage() {
  const { testNets, eip155Address, cosmosAddress, solanaAddress, liskAddress } = useSnapshot(SettingsStore.state)
  ...
  return (
    <Fragment>
      ...
      {Object.values(LISK_MAINNET_CHAINS).map(({ name, logo, rgb }) => (
        <AccountCard key={name} name={name} logo={logo} rgb={rgb} address={liskAddress} />
      ))}
      ...
      {testNets ? (
        <Fragment>
          ...
          {Object.values(LISK_TEST_CHAINS).map(({ name, logo, rgb }) => (
            <AccountCard key={name} name={name} logo={logo} rgb={rgb} address={liskAddress} />
          ))}
        </Fragment>
      ) : null}
